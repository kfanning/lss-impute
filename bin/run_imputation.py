...
#run imputation
if uargs.stack_catalogs:
    cat2n = obs_nncat_s
    cat2s = obs_nncat_n #these are flipped intentionally! I want to pass both catalogs to the class!
else:
    cat2n = None
    cat2s = None
...
imps_cat = imps.run(skip_background=uargs.nobackground, physical=uargs.physical, fit=uargs.fit, rbins=uargs.radial_bins, angbins=uargs.perp_bins, fit_type=uargs.fit_type, extended=uargs.extend, clusfrac_suppression=cf_factor)
figs = imps.figs

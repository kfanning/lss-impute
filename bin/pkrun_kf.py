...
from xirunpc_kf import read_clustering_positions_weights, concatenate_data_randoms, compute_angular_weights, catalog_dir, get_regions, get_zlims, get_scratch_dir
...
def power_fn(file_type='npy', region='', tracer='ELG', tracer2=None, zmin=0, zmax=np.inf, rec_type=False, weight_type='default', bin_type='lin', out_dir='.', impute_type=None, impute_number=None):
    if tracer2: tracer += '_' + tracer2
    if rec_type: tracer += '_' + rec_type
    if region and isinstance(region, str): tracer += '_' + region
    if impute_type: tracer += '_' + impute_type
    if impute_number: tracer += '_' + str(impute_number)
...
def window_fn(file_type='npy', region='', tracer='ELG', tracer2=None, zmin=0, zmax=np.inf, rec_type=False, weight_type='default', bin_type='lin', out_dir='.', impute_type=None, impute_number=None):
    if tracer2: tracer += '_' + tracer2
    if rec_type: tracer += '_' + rec_type
    if region and isinstance(region, str): tracer += '_' + region
    if impute_type: tracer += '_' + impute_type
...
if __name__ == '__main__':
    ...
    parser.add_argument('--survey', help='e.g., SV3 or main', type=str, choices=['SV3', 'DA02', 'main','SV3mock', 'y1mock'], default='SV3')
    ...
    if mpicomm is None or mpicomm.rank == mpiroot:
        ...
        logger.info(f'Impute type: {impute_type}, impute number: {impute_number}')
    ...
    for zmin, zmax in zlims:
        base_file_kwargs = dict(tracer=tracer, tracer2=tracer2, zmin=zmin, zmax=zmax, rec_type=args.rec_type, weight_type=args.weight_type, bin_type=bin_type, out_dir=os.path.join(out_dir, 'pk'), impute_type=impute_type, impute_number=impute_number)

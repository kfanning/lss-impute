...
def catalog_dir(survey='main', verspec='guadalupe', version='test', base_dir='/global/cfs/cdirs/desi/survey/catalogs'):
    return os.path.join(base_dir, survey, version)
    ...
    def read_positions_weights(name):
        ...
        for reg in region:
            ...
            if name == 'random' and rand_override and imp:
                ...
                cat_fns = catalog_fn(ctype='clustering', name=name, region=reg, **kwargs2)
            else:
            ...
            if name == 'data' and imp:
                isscalar = False
                logger.info(f'Loading imputation file for type {kwargs["impute_type"]}, {kwargs["impute_number"]}')
                kwargs2 = kwargs.copy()
                olddir = Path(kwargs['cat_dir'])
                kwargs2['cat_dir'] = os.path.join(olddir.parents[1], kwargs['impute_type'], kwargs['impute_number'])#os.path.join(olddir, '..',  '..', kwargs['impute_type'], kwargs['impute_number'])
                #kwargs2['survey'] = kwargs['impute_type']
                #kwargs2['version'] = kwargs['impute_number']
                #kwargs2['cat_dir'] = None
                cat_fn2 = catalog_fn(ctype='clustering', name=name, region=reg, **kwargs2)
                cat_fns.append(cat_fn2)
            logger.info('Loading {}.'.format(cat_fns))
            logger.info(f'isscalar = {isscalar}')
            ...
            if isscalar or (len(positions_weights)==1):
                ...
                if concatenate or ((imp is not None) and (name=='data')):
                    p, w = _concatenate(p), _concatenate(w)
                positions.append(p)
                weights.append(w)
            logger.info(f'Length of positions: {[len(p) for p in positions]}, {[len(q) for q in positions[0]]}')
        return positions, weights
...
def corr_fn(file_type='npy', region='', tracer='ELG', tracer2=None, zmin=0, zmax=np.inf, rec_type=False, weight_type='default', bin_type='lin', njack=0, nrandoms=8, split_randoms_above=10, out_dir='.', option=None, impute_type=None, impute_number=None):
    ...
    if impute_type is not None:
        imp = impute_type + str(impute_number)
    else:
        imp = ''
    split = '_split{:.0f}'.format(split_randoms_above) if split_randoms_above < np.inf else ''
    root = '{}_{}_{}_{}_{}_{}_njack{:d}_nran{:d}{}'.format(tracer, imp, zmin, zmax, weight_type, bin_type, njack, nrandoms, split)
    if file_type == 'npy':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--basedir', help='where to find catalogs', type=str, default='/global/cscratch1/sd/kfanning/impute/catalogs/')
    ...
    parser.add_argument('--region', help='regions; by default, run on N, S; pass NS to run on concatenated N + S', type=str, nargs='*', choices=['N', 'S', 'NS'], default='NS')
    parser.add_argument('--zlim', help='z-limits, or options for z-limits, e.g. "highz", "lowz", "fullonly"', type=str, nargs='*', default='fullonly')
    ...
    if True: #os.path.normpath(args.basedir) == os.path.normpath('/global/cfs/cdirs/desi/survey/catalogs/'):
    ...
    for zmin, zmax in zlims:
        base_file_kwargs = dict(tracer=tracer, tracer2=tracer2, zmin=zmin, zmax=zmax, rec_type=args.rec_type, weight_type=args.weight_type, bin_type=args.bin_type, njack=args.njack, nrandoms=args.nran, split_randoms_above=args.split_ran_above, impute_type=impute_type, impute_number=impute_number, option=option)
        ...
        for corr_type in args.corr_type:
            ...
            if mpicomm is None or mpicomm.rank == mpiroot:
                ...
                for region in all_regions:
                    ...
                    txt_kwargs.update(region=region, out_dir=os.path.join(out_dir, corr_type), impute_type=impute_type, impute_number=impute_number)
